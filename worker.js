// ===========================
// MTCweb Gallery & File Depot (Local API + CF Cache Version)
// ===========================
...
export default {
  async fetch(request, env, ctx) {
    ...
    // 文件代理 (处理图片 file_id 和 视频直链 URL)
    if (path.startsWith("/file/")) {
      const param = decodeURIComponent(path.replace("/file/", ""));
      // 将 Range 头透传给 proxy 函数，用于视频拖拽播放
...
/**
 * 核心代理函数：支持 Telegram 官方 file_id 和 Leaflow 直链 URL
 */
async function proxyTelegramFile(env, fileIdOrUrl, url, rangeHeader) {
  // 模式 A: 处理 HTTP 直链 (来自 Leaflow Local API) -> 开启 Cloudflare 缓存
  if (fileIdOrUrl.startsWith("http")) {
    try {
      const targetUrl = fileIdOrUrl;

      // 构造回源请求
      const newReqHeaders = new Headers();
      if (rangeHeader) newReqHeaders.set("Range", rangeHeader); // 透传 Range，支持视频拖拽
      ...
      // 发起请求，并配置 Cloudflare 缓存策略
      const response = await fetch(newReq, {
        cf: {
          cacheTtl: 14400, // 缓存 4 小时 (足够看完一部电影)
          cacheEverything: true // 强制缓存非静态文件
        }
      });
      ...
      newHeaders.set("Access-Control-Allow-Origin", "*"); // 允许跨域播放
  ...
  // 模式 B: 处理 Telegram 官方 file_id (原有逻辑，主要用于小图片)
...
function renderPagination(current, totalPages, basePath) {
  ...
  if (totalPages <= 7) {
    for (let i = 1; i <= totalPages; i++) pageLink(i, null, false, i === current);
  } else {
...
async function renderList(env, url, categories) {
  ...
  const currentPage = Math.min(totalPages, Math.max(1, parseInt(url.searchParams.get("page") || "1", 10)));
...
async function renderCategoryPage(env, url, categories, currentCat) {
  ...
  const paginationHtml = renderPagination(currentPage, totalPages, `/category/${encodeURIComponent(currentCat.replace(/\s+/g, '-'))}`);
...
async function renderAlbum(env, code, url) {
  ...
  if (prevCode) { const p = await env.ALBUMS.get(prevCode, { type: "json" }); prevTitle = p ? (p.title || prevCode) : prevCode; }
  if (nextCode) { const n = await env.ALBUMS.get(nextCode, { type: "json" }); nextTitle = n ? (n.title || nextCode) : nextCode; }
  ...
  const atts = data.attachments.map(att => {
      let link, target, icon, text;

      // 1. 优先判断是否有直链 (Leaflow 缓存文件) -> 走高速代理
      if (att.direct_url) {
          // 构造代理链接，让 Worker 去 fetch 直链
          link = `${url.origin}/file/${encodeURIComponent(att.direct_url)}?download=${encodeURIComponent(att.file_name)}`;
          target = ''; // 不需要新窗口，直接下载/播放
          icon = '⚡️'; // 高速图标
          text = escapeHtml(att.file_name) + ' (高速直连)';
      } 
      // 2. 其次判断是否有 TG 链接 (旧数据) -> 跳转
      else if (att.tg_link) {
          link = att.tg_link;
          target = 'target="_blank"';
          icon = '✈️';
          text = `Open in Telegram: ${escapeHtml(att.file_name)}`;
      } 
      // 3. 兜底 (普通 file_id) -> 走普通代理 (仅限<20MB)
      else {
          link = `${url.origin}/file/${att.file_id}?download=${encodeURIComponent(att.file_name)}`;
          target = '';
          icon = '📄';
          text = escapeHtml(att.file_name);
      }
  ...
  if (data.zip) {
      ...
      if (data.zip.direct_url) {
          link = `${url.origin}/file/${encodeURIComponent(data.zip.direct_url)}?download=${encodeURIComponent(data.zip.file_name)}`;
          target = '';
          text = '📦 Download Zip (High Speed)';
      } else if (data.zip.tg_link) {
          link = data.zip.tg_link;
          target = 'target="_blank"';
          text = '📦 Get Zip in Telegram';
      } else {
          link = `${url.origin}/file/${data.zip.file_id}?download=${encodeURIComponent(data.zip.file_name)}`;
          target = '';
          text = '📦 Download Zip';
      }
  ...
  <div class="bottom-nav">
     ${prevCode ? `<a href="/${prevCode}" class="nav-btn prev"><span class="nav-label">« PREV</span><span class="nav-name">${escapeHtml(prevTitle)}</span></a>` : `<div></div>`}
     ${nextCode ? `<a href="/${nextCode}" class="nav-btn next"><span class="nav-label">NEXT »</span><span class="nav-name">${escapeHtml(nextTitle)}</span></a>` : `<div></div>`}
  </div>`;
  ...
  const html = `
    ...
    <main class="main">
        ...
        ${zipHtml}
        ${imgs}
        ${atts ? `<div style="margin-top:30px;padding:16px;border:1px solid #334155;border-radius:12px;background:#0f172a;"><h3 style="margin-top:0;color:#e11d48;">Resources & Downloads</h3><ul style="padding:0;margin:0;">${atts}</ul></div>` : ""}
        ${navHtml}
    </main>
